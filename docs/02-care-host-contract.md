...
| Care's DRF exception handler is `config.exception_handler.exception_handler`, which wraps DRF's own. DRF's handler calls `set_rollback()` for every `APIException`, and `set_rollback()` marks the request transaction for rollback when `ATOMIC_REQUESTS` is on. So a plug view that **raises** a DRF `ValidationError` after it wrote a row (an `AbdmOutboundRequest` for a refused registry call) loses that row; a view that **returns** a `Response` with the same status keeps it. Observed 2026-09-21 (findings J9). | `~/ohc.network/care/config/settings/base.py:384`; `~/ohc.network/care/config/exception_handler.py:7-11`; `rest_framework/views.py:66-69,99` (DRF in Care's venv) |
...
| The host and every remote share 1 `raviger` 5.3 (`vite.config.mts` shared list). Its `useQueryParams` setter serialises through `URLSearchParams` and drops only `null`, so a key set to `undefined` is written as the string "undefined" (findings J11). | `frontend/node_modules/raviger/dist/module.js` (`serializeQuery`: `Object.entries(t).filter(([,t]) => null !== t)`) |
| The host's own list pages never hit this: `useFilters` cleans every query with `FiltersCache.utils.clean` (a key stays only when `(value ?? "") != ""`) and writes with `replace: true`. A remote cannot import the hook (no host hooks are shared), so the plug mirrors the rule in `frontend/src/lib/query-params.ts`. | `~/ohc.network/care_fe/src/hooks/useFilters.tsx:48-58`; `~/ohc.network/care_fe/src/Utils/FiltersCache.tsx:17-27` |
| The host has no dark mode today: Tailwind is set to `darkMode: ["class"]`, `next-themes` is a dependency, but no `ThemeProvider` is mounted (0 uses in `src`) and nothing adds `dark` to `<html>`; the only `useTheme()` call is the toaster's, which falls back to `"system"`. So a plug `dark:` utility fires only inside an element that itself carries `dark` — which is how the developer console is skinned (ADR-018 amendment). | `~/ohc.network/care_fe/tailwind.config.js:16`; `~/ohc.network/care_fe/package.json:126`; `~/ohc.network/care_fe/src/components/ui/sonner.tsx:1-11` |
| The plug's own `.dark` token block is scoped by the PostCSS pass to `.care-abdm-fe-container .dark`, so the tokens apply to a `.dark` element inside the plug root only, never to a host-level class. `.dark.abdm-console` on 1 wrapper is therefore complete in itself. | `frontend/scripts/postcss-scope-plugin.ts:76-98`; `frontend/src/style/index.css:108` |
| The federation runtime appends the remote stylesheet as `<link rel=stylesheet href="<remoteEntry dir>/style-*.css">`, so a URL inside that CSS resolves against the remote origin only when it is relative. Vite's default writes `/assets/…` (absolute, host origin → 404); `experimental.renderBuiltUrl` with `hostType === "css"` → `{relative: true}` fixes it. A font is CORS-restricted: the remote must answer the woff2 with `Access-Control-Allow-Origin: *` (the preview does: observed 2026-09-22). | `frontend/node_modules/@originjs/vite-plugin-federation/dist/index.js:1258-1270` (css bundle list); the built `remoteEntry.js` (`document.createElement("link")`, `href` from `import.meta.url`); `frontend/vite.config.ts:52` |
...
### M4 host facts (read 2026-09-19; ADR-015)

| Host fact | Source |
|---|---|
| A `userNavItems` entry renders in the user's own menu and navigates to `/facility/${selectedFacilityId}/users/${user.username}/${item.url}`; `item.name` goes through the host `t()`. **There is no fallback for the state with no facility** (the host's own "Profile" item has one), so such a link goes to `/facility/undefined/...` outside a facility. The plug declares no `userNavItems` (ADR-017). | `~/ohc.network/care_fe/src/components/ui/sidebar/nav-user.tsx:45-47,123-134` |
| The user profile summary has a plug slot: `UserProfileSections`, `React.FC<{user: UserRead; isOwnProfile: boolean; className?: string}>`, rendered at the end of the page inside the page container. Each host section of that page is a `UserColumns`: `<section className="flex flex-col gap-5 sm:flex-row">`, a `sm:w-1/4` heading and note, a `sm:w-3/4` body. The plug section copies that shape. | `~/ohc.network/care_fe/src/pluginTypes.ts:141-146`; `~/ohc.network/care_fe/src/components/Users/UserSummary.tsx:203-207`; `~/ohc.network/care_fe/src/components/Common/UserColumns.tsx` |
| `UserHome` shows `ErrorPage` for a tab it does not know, so a plug page under the user menu must claim the static path itself. Plug routes are spread before the host routes and raviger takes the first match. The plug no longer uses this path (ADR-017). | `~/ohc.network/care_fe/src/components/Users/UserHome.tsx:95-102`; `~/ohc.network/care_fe/src/Routers/AppRouter.tsx:25-32`; `~/ohc.network/care_fe/src/Routers/routes/UserRoutes.tsx:9-13` |
| `Facility` has `name`, `address`, `pincode`, `phone_number`, `latitude`, `longitude`; the HFR wizard prefills from them. | `~/ohc.network/care/care/facility/models/facility.py:154-190` |

### Facility creation and component overrides (read 2026-09-19; ADR-016)

| Host fact | Source |
|---|---|
| Care creates a facility through `FacilityViewSet.handle_create(data)`: `FacilityCreateSpec.model_validate` with `context={"is_create": True}`, `validate_data`, `authorize_create` (`can_create_facility`), `de_serialize`, `perform_create` (sets `created_by`, saves). `Facility.save()` on create makes the root `FacilityOrganization` "Administration" and the creator its facility admin, then `sync_cache()`. A plug can run the same path in-process by instantiating the viewset with the request. | `~/ohc.network/care/care/emr/api/viewsets/base.py:95-153`; `~/ohc.network/care/care/facility/models/facility.py:241-256` |
| `FacilityCreateSpec`: `name` (unique, normalised), `description`, `facility_type` (a type **name** from `REVERSE_REVERSE_FACILITY_TYPES`), `features: list[int]`, `pincode: int`, `address`, `phone_number: str` (the host form validates E.164), `latitude`, `longitude`, `geo_organization: UUID4` (a `govt` organization), `is_public`. Core viewsets turn pydantic errors into `{"errors": [{loc, msg, ...}]}` 400. | `~/ohc.network/care/care/emr/resources/facility/spec.py:100-175`; `~/ohc.network/care/care/emr/api/viewsets/base.py:26-45` |
| `FACILITY_TYPES` (int, name) and `FacilityFeature` (IntegerChoices, 6 values) are the choice lists; the host frontend copies them into `constants.tsx`, which remotes cannot import. | `~/ohc.network/care/care/facility/models/facility.py:25-82` |
| Government organizations: `org_type="govt"`, tree by `parent`, `level_cache`, `has_children`; `metadata` holds `govt_org_type` and `govt_org_children_type` only, **no LGD codes**. Public list `GET /api/v1/govt/organization/?parent=<id>` (empty `parent` lists the roots; `name`, `org_type`, `level_cache` filters; `limit` ≤ 200), detail `GET /api/v1/govt/organization/<id>/` with the parents nested in `parent`. The host facility form accepts a **leaf** only. | `~/ohc.network/care/care/emr/api/viewsets/organization.py:49-68`; `~/ohc.network/care/care/emr/models/organization.py:23-60`; `~/ohc.network/care/config/api_router.py:195`; `~/ohc.network/care_fe/src/components/Facility/FacilityForm.tsx:110-114` |
| The host lists an organization's facilities with `GET /api/v1/facility/?organization=<id>` = `geo_organization_cache__overlap=[org.id]`, over the caller's visibility (`OrganizationUser` and `FacilityOrganizationUser` memberships; superuser sees all). | `~/ohc.network/care/care/emr/api/viewsets/facility.py:60-103` |
| Component overrides: the host build wraps every exported PascalCase component named in `REACT_MFE_REGISTERED_COMPONENTS` (comma list; empty = none; `*` = all; unknown name = build error) with `register(key, Base)`. A plug replaces 1 by declaring `overrides: [{component, replacement, condition?, priority?, description?}]` in its manifest; `PluginEngine` calls `addOverride` and cleans up on reload. The replacement receives the base props plus `__base`. An override that throws falls back to the base. `window.__careOverrides.addComponent` is the bridge for plugs that register outside the manifest (care_govt_hmis_fe `componentOverrides.ts`). | `~/ohc.network/care_fe/plugins/autoRegisterComponents.ts`; `~/ohc.network/care_fe/docs/care-apps-plugin-overrides.md`; `~/ohc.network/care_fe/src/PluginEngine.tsx:130-150`; `~/ohc.network/care_fe/src/lib/override/{register,bridge}.ts` |
| `AddFacilitySheet` (`export default function`, props `{organizationId}`) renders on the organization facilities page when `canCreateFacility`; it is a supported export form for the override transform. There is no additive slot on that page. | `~/ohc.network/care_fe/src/pages/Organization/components/AddFacilitySheet.tsx:23`; `~/ohc.network/care_fe/src/pages/Organization/OrganizationFacilities.tsx:87` |
| `organizationTabs: [{name, slug, icon, component: FC<{contextId, navOrganizationId?}>}]` renders a plug tab on the organization page at `/organization/:id/<slug>` (or `/organization/:navId/children/:id/<slug>`). Not used by the plug since 2026-09-19 (the facility overview lives on `/admin/abdm`). | `~/ohc.network/care_fe/src/pages/Organization/components/OrganizationLayout.tsx:71-72,110-112,156-157`; `pluginTypes.ts:181-186` |
| Remotes share only `react`, `react-dom`, `react-i18next`, `@tanstack/react-query`, `raviger`, `sonner`, `decimal.js` with the host: no host components, hooks or constants. | `~/ohc.network/care_fe/vite.config.mts:445-453` |
