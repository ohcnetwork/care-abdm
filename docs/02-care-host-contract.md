...
| Host fact | Source |
|---|---|
| A `userNavItems` entry renders in the user's own menu and navigates to `/facility/${selectedFacilityId}/users/${user.username}/${item.url}`; `item.name` goes through the host `t()`. | `~/ohc.network/care_fe/src/components/ui/sidebar/nav-user.tsx:45-47,123-134` |
| `UserHome` shows `ErrorPage` for a tab it does not know, so a plug page under the user menu must claim the static path itself. Plug routes are spread before the host routes and raviger takes the first match, so `/facility/:facilityId/users/:username/abdm-hpr` wins over the host's `:tab` route. | `~/ohc.network/care_fe/src/components/Users/UserHome.tsx:95-102`; `~/ohc.network/care_fe/src/Routers/AppRouter.tsx:25-32`; `~/ohc.network/care_fe/src/Routers/routes/UserRoutes.tsx:9-13` |
