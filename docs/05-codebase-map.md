...
| `models.py` | M1: `AbhaTransaction`, `AbdmProfileShare`. Audit: `AbdmOutboundRequest`, `AbdmCallback`. M2: `AbdmLinkToken`, `AbdmCareContext` (`hi_types` derived from the linked items), `AbdmShareItem` (ADR-013: 1 row per shareable record, `staged → queued → linked`, or `failed` / `excluded`), `AbdmLinkSession`, `AbdmConsent`, `AbdmDataRequest` | Secrets (X-token, link token, OTP hash) stay here, never in an extension. Ciphertext is never stored |
| `migrations/` | `0001_initial` (rebased with Care develop 2026-09-17), `0002_abdmshareitem` (adds the table and turns every advertised `hi_types` value into a linked item) | Must match `models.py` |
...
| `tasks.py` | `dispatch_callback` (verified callback → handler); `stage_record` (stage 1 record); `sync_encounter` (Encounter change; close → link); `link_care_context` (1 link call for the queued items); `notify_care_context` (delayed notify, F9); `retry_share_items` (periodic, every 5 min: re-queue due items, ADR-013 D4) | A Celery worker must run for M2 and for Scan and Share. A refusal is never repeated: it earns `ABDM-1092` |
...
| File | Job |
|---|---|
| `manifest.tsx` | Routes `/facility/:facilityId/abdm/setup` and `/admin/abdm` (+ `adminNavItems` entry "ABDM"); `encounterTabs.abdm` (the host owns the label `ENCOUNTER_TAB__abdm`); slots `FacilityHomeActions`, `PatientRegistrationForm`, `PatientDetailsTabDemographyGeneralInfo`, `PatientHomeActions`, `PatientSearchActions`, `EncounterActions`, `EncounterOverviewTop` |
...
| `components/abdm/encounter-tab.tsx` | Encounter tab "ABDM Records" (ADR-013 D5): header card (display, status, ABHA address, reference, shared types, counts, failure), records table (select, label, type, status + next attempt, attempts, exclude/include), "Link selected" / "Link all staged". Append-only layout |
