...
| `facility/service.py` | Reads and writes `Facility.extensions["abdm"]`; `hip_id_for()` derives HIP ID = HFR facility ID; `facility_for_hip_id()` resolves an inbound HIP ID in either form (`IN…` or the gateway's `IN…_1`); `register_hrp_service()` | `hip_id_for()` and `facility_for_hip_id()` are the only HIP ID logic |
