...
Sandbox test data is in the [data dictionary](/docs/hiecm/v3/reference/data-dictionary). [Support](/docs/support) lists the channels. The cases you are certified against are in the certification pack NHA issues.
...
The card is returned in the response rather than fetched from a separate link. The content type and encoding of the card and QR code responses are not yet published. See the [APIs](/docs/hiecm/v3/api/m1) page for the fields that are.
...
The calls are the enrolment set in [the P1 reference](/docs/hiecm/v3/milestones/p1), written up as [creating an ABHA address](/docs/hiecm/v3/milestones/p1#creating-an-abha-address).
...
Once it verifies, the [PHR profile](/docs/hiecm/v3/api/m1/endpoints/m1-login-abha-address-mobile-otp/04-m1-get-v3-phr-web-login-profile-abha-profile) and the PHR card and QR code read against the token it returns.
...
The two calls: [receive a patient's shared profile](/docs/hiecm/v3/api/m2/endpoints/m2-abdm-patient-share-hip/01-m2-post-v3-hip-patient-share) and [send the share acknowledgement](/docs/hiecm/v3/api/m2/endpoints/m2-abdm-patient-share-hip/02-m2-post-patient-share-v3-on-share). The patient's side is [P2 Linking and records](/docs/hiecm/v3/milestones/p2#scan-and-share-at-a-facility).
