...
| B5 | 2026-09-09 | Resolved 2026-09-15 (sandbox) | Whether the bridge `url` may carry a path prefix is unstated. It may: the gateway stored `https://care-abdm-sbx.rithviknishad.dev/api/abdm` and **delivered callbacks to `{bridgeUrl}/v3/hip/token/on-generate-token` with the prefix** (several `Internal Server Error: /api/abdm/v3/hip/token/on-generate-token` lines in the Care log between 16:20 and 17:50 IST; only 1 of them was our own probe). The docs page should say the URL may carry a path and that the callback path is appended to it. | `gateway-update-bridge-url` |
...
| E10 | 2026-09-15 | Open | Retry after a failed delivery is undocumented ("Whether and when delivery attempts stop is not documented yet", troubleshooting). Observed: while our receiver answered 500 (B14 + J6), the gateway delivered several callbacks; after the receiver was fixed at 17:5x IST, a new accepted `m2-generate-link-token` (17:51:20, REQUEST-ID `65398090-4b38-4475-8330-b6ae2fa2b2a6`) produced no delivery in the first 15 minutes, and no earlier callback was retried. Whether the gateway backs off or pauses a bridge after 5xx answers should be stated. | `troubleshooting/callback-never-arrives` |
