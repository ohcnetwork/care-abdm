...
- [~] 1. Facility and bridge.
  ...
  - [ ] USER: `manage.py abdm_register_bridge_url` after `migrate abdm` (the tables were reset), then HRP register with the exact HFR name and a ≤15-character HIP name → `hrp_registered_at` set; `GET /api/abdm/bridge` shows 1 service.
  - [ ] USER screenshot: `/facility/<id>/abdm/setup` shows the 3 cards and saves.
