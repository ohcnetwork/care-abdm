...
- [~] 3. Link token: `on-generate-token` lands for a patient with ABHA → `AbdmLinkToken.status=active`; record the token's `exp` claim (findings F2).
  ...
  - [ ] USER: in the runserver terminal, count the `Internal Server Error: /api/abdm/...` tracebacks between 16:20 and 17:50 IST and note their timestamps. 1 (about 17:37) = the probe only, so the gateway never called. More than 1 = the gateway did call and the receiver's certs bug (fixed 17:5x) destroyed the row. Also search for `Not Found: /v3/` or `Not Found: /api/v3/`.
  - [ ] USER: press "Link to ABHA → Link now" once more (the receiver is fixed now), then watch `/admin/abdm → Recent gateway callbacks` for 15 minutes. Any row, even a red one, is evidence: it carries the header name and the body.
