...
- [~] 3. Link token: `on-generate-token` lands for a patient with ABHA → `AbdmLinkToken.status=active`; record the token's `exp` claim (findings F2).
  ...
  - [x] Found 2026-09-15 (user's worker log + `diag_fork.py`): the Celery prefork child segfaulted on every `sync_encounter` run (`signal 11`). Cause: macOS fork-unsafe libpq (GSSAPI) and `_scproxy`. Fix: worker env `PGGSSENCMODE=disable no_proxy='*'` (`04-dev-setup.md`). Until now no Celery-run ABDM call or callback handler could have completed on this machine; the 16:20 call worked because the API request ran it inline.
  - [ ] USER: restart the worker with the 2 variables, then press "Link to ABHA → Link now" on the encounter (or change its status) and watch `/admin/abdm` → Recent callbacks.
