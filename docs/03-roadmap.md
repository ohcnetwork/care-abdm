...
- [~] 3. Link token: `on-generate-token` lands for a patient with ABHA → `AbdmLinkToken.status=active`; record the token's `exp` claim (findings F2).
  ...
  - [x] Observed 2026-09-15 17:37 IST (user, runserver log): `Internal Server Error: /api/abdm/v3/hip/token/on-generate-token` — a POST **did reach the documented callback path** on the bridge. The receiver crashed: `GatewayCertsError: gateway certs failed: HTTP 401` (the certs endpoint needs the bearer token, findings B14), and Care's `ATOMIC_REQUESTS` rolled the stored row back (J6). Fixed the same hour: certs fetch carries the token; the receiver never raises (401 bad signature, 503 verifier unavailable, row kept with `signature_error`); the verifier accepts the JWK's own RS256/RS512 and auto-detects the header (migration 0002). Smoke: 503 + row kept; real verifier 202 with auto-detected header; bad signature 401 with reason. The agent's report settled it: that POST was the agent's own public probe (it received the 500 page), which proves Cloudflare → tunnel → Care → plug for POST. No real gateway callback has been observed yet.
  - [x] Observed 2026-09-15 17:36–17:42 IST (agent, 1 call): `m2-generate-link-token` with `X-HIP-ID: IN1410000232_1` (the gateway service id) → 202, REQUEST-ID `18124b23-7f6f-41af-ba00-228518245d0c`; no callback in 6 minutes. A POST to a path outside the plug (`/v3/...` on the bare host) answers 404 from Care, so a URL-join bug at the gateway would show as `Not Found: /v3/...` in the runserver log.
  - [x] Observed 2026-09-15 (user): **more than 1** `Internal Server Error: /api/abdm/...` between 16:20 and 17:50 IST. The gateway did call the documented path on the prefixed bridge URL (findings B5 resolved) and the receiver's certs bug destroyed every row (J6). Exact timestamps still to be read off the log to measure request → callback latency.
  - [x] Observed 2026-09-15 17:51:20 IST (user, "Link now" after the fix): `m2-generate-link-token` → 202, REQUEST-ID `65398090-4b38-4475-8330-b6ae2fa2b2a6`. No POST from the gateway reached Care by 18:05 (the runserver log shows every request; none on `/api/abdm/`). Bridge live at 18:06: `active: true, blocklisted: false`. A public probe at 18:07 → 401 with the row stored (`signature_error` "No JWT in header…", `Cf-Connecting-Ip` present), so the fixed receiver is reachable. Recorded as findings E10 (retry after 5xx undocumented).
  - [x] Observed 2026-09-15 18:12–18:18 IST (user, repeated "Link now"): `8e797fc6` accepted (202), then 7 refusals `HTTP 400 ABDM-1092 Duplicate Link token request` with fresh REQUEST-IDs (findings F6). The plug now keeps the token row `requested` on 1092, pointing at the last accepted request, and shows the desk "The gateway still holds an open link-token request… Waiting for its callback". The real row was repaired by hand (it had flipped to `failed` and pointed at a refused request).
  - [ ] USER: paste the exact `[15/Sep/2026 HH:MM:SS] "POST /api/abdm/v3/hip/token/on-generate-token HTTP/1.1" 500` lines from the runserver log so the gateway's request → callback latency is known.
  - [ ] Wait for the gateway. Check `/admin/abdm → Recent gateway callbacks` after 30 and 60 minutes. Any row (even red) closes E2 (header name) and shows the body and the gateway's source IP.
  - [ ] If still nothing: ask the docs team whether the sandbox delivers M2 callbacks to a bridge URL that carries a path (`/api/abdm`), and escalate on the NHA forum with the API, REQUEST-ID and TIMESTAMP as the docs' troubleshooting page prescribes.
