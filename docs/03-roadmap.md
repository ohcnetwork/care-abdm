...
- [x] 3. Link token. **Root cause of "no callbacks" found 2026-09-17:** every request since 2026-09-15 sent `X-HIP-ID: IN1410000232` (the HFR id, as the docs say) and the gateway accepts that with 202 but never delivers the callback. The gateway routes on the service id it issued at HRP registration, `IN1410000232_1`; with that header the callback arrived in 1.9 s and the token's own claims say `hipId: IN1410000232_1` (findings E1, B18). The plug now stores the service id read from `gateway-list-bridge-services` (after HRP registration and on every setup-page load) and a facility without one is not configured for M2. Token: RS512 JWT, `exp` = `iat` + 182.5 days (F2). The old 2026-09-15 sub-bullets are superseded by this one.
  ...
  - [x] **2026-09-17 11:46Z to 11:56Z: the empty 400 is an ABDM fault, not ours (E11).** 9 controlled calls on `nihal_99@sbx` with the 6-month link token of row 13. (1) `patient: []` answers JSON `400 ABDM-9999: patient attribute required in the payload`, so ABDM reads our payload. (2) An invalid `hiType` answers the same empty 400 and not `ABDM-1006`, so a well formed request never reaches field validation. (3) 1 call answers `500 303001 Runtime Error … [ Name : HIECM-HIP-INITIATED-LINKING--vv3_APIproductionEndpoint ] [ State : SUSPENDED ]`, which is the API manager circuit breaker. Every refusal comes from `server: istio-envoy` in 16-25 ms and carries `correlation-id` and `activityid`. The plug now stores the response headers of each refusal (`AbdmOutboundRequest.response_headers`, migration `0003`). Nothing on our side is left to vary.
  - [ ] **Escalate to NHA** with the `correlation-id`, `activityid` and `REQUEST-ID` values in `AbdmOutboundRequest`, and ask when `HIECM-HIP-INITIATED-LINKING--vv3` is healthy again.
  - [ ] **After 2026-09-18 07:30Z (block lifted) and after NHA repairs the endpoint:** on one encounter press "Share with ABHA" once. Expected: token callback within seconds → link call → `on_carecontext` callback → `linked` → notify → `on-notify`. If the link still answers an empty 400 with a seconds-old token, escalate on the docs forum with the REQUEST-IDs in `AbdmOutboundRequest` (E11); nothing else on our side is left to vary.
- [ ] 4. HIP link: `on_carecontext` lands → `AbdmCareContext.status=linked`; record the real `status` value (findings F4). Blocked on the ABDM-side outage above (E11), not on plug code.
...
- [ ] Still open: `m2-hip-link-care-context` has never succeeded (9 of 9 empty HTTP 400, finding E11).
      ADR-012 makes that visible and gives support the reference; it does not cure it.
